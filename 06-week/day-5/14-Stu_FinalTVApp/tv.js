  ...
  this.findActor = function(actor) {
    ...
    axios.get(URL).then(
      function(response){
          ...
          var actorResponse = response.data[0].person;
          var actorData = [
            "Actor Name: " + actorResponse.name,
            "Birthday: " + moment(actorResponse.birthday).format("LL"),
            "Gender: " + actorResponse.gender,
            "Country: " + actorResponse.country.name,
            "URL: " + actorResponse.url
          ].join("\n\n");
          fs.appendFile("log.txt", actorData + divider, function(err) {
            if (err) throw err;
          })
          console.log(actorData + divider);
      }
  )
